          ...
          {Object.entries(questions[index].options).map(([key, val]) => {
            return (
              <div
                ...
                className={` h-[111px] rounded-[20px] text-[20px] flex items-center text-white p-4 cursor-pointer hover:bg-copper hover:border-solid hover:border-4 hover:border-white ${
                  answer[index + 1] === key
                    ? "bg-copper border-solid border-4 border-white"
                    : "bg-pastelPink"
                }`}
              >
                {`${key}. ${val}`}
